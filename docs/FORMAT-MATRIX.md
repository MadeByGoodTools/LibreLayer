...
| Camera RAW: CR2, CR3, NEF, ARW, DNG, RAF, ORF, RW2 | Yes for supported Bayer/X-Trans sensor data | Developed image / 16-bit TIFF master | Opens into the local RAW developer with a persisted recipe, deterministic detail/noise repair, local focal-family optics profiles, and optional Smart Object source recovery. Camera coverage is not yet a complete vendor/model matrix. |
