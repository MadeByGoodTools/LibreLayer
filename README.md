...
- Portable `.libreRAW.json` Camera Raw sidecars preserve every validated decode, color, noise, and optics recipe without embedding the original image data; sequential local batch development reuses that recipe and exports one PNG at a time with safe stopping and per-file failure reporting
...
- Fractional brush-stroke interpolation plus a batched WebGL2 subpixel renderer with Canvas fallback, angle, roundness, pressure/tilt, transfer jitter, dual-tip, wet-edge, airbrush, scatter, texture, and Mixer Brush controls saved in brush presets
- Dedicated Liquify, Adaptive Wide Angle, and Vanishing Point workspaces with live previews, draggable controls, selection-aware output, and reusable displacement meshes
...
The [user manual](docs/USER-MANUAL.md), [plug-in platform specification](docs/PLUGIN-PLATFORM.md), [filter plug-in specification](docs/FILTER-PLUGINS.md), [background-job safety model](docs/BACKGROUND-JOBS.md), [format matrix](docs/FORMAT-MATRIX.md), [privacy and storage model](docs/PRIVACY-AND-STORAGE.md), [migration policy](docs/MIGRATION-POLICY.md), [security policy](SECURITY.md), and [0.1.0 release-candidate notes](docs/RELEASE-NOTES-0.1.0.md) document the current verified scope and limits.
