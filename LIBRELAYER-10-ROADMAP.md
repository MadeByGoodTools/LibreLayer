...
- [x] GPU subpixel brush renderer with stable low-latency interpolation.
...
- [x] Native text, shape, fill, adjustment, effect, Smart Object, and Smart Filter PSD round-trip for the explicitly supported records below; unsupported richer records fail closed.
  ...
  - [x] Native editable PSD shape layers with closed Bézier paths, Boolean path records, solid fill, simple stroke, rendered fallback pixels, project persistence, codec round-trip tests, and explicit blocking for unsupported vector content.
...
- [x] True embedded source documents editable in their own tabs.
...
- [x] Nondestructive skew, perspective, warp, vector, RAW, and nested object content.
...
- [x] Tiled document backing, mipmaps, dirty regions, cached composites, and GPU texture pooling.
  ...
  - [x] Bounded WebGL2 framebuffer and texture pooling for brush tiles, with validated targets, per-tile fallback, deterministic release, and GPU-memory eviction.
...
- [x] Artboards, frames, key-object alignment, smart spacing, and multi-scale asset export.
...
- [x] Variables, datasets, layer/artboard export, contact sheets, panorama, HDR merge, and focus stack.
- [x] Auto-align, Auto-Blend, image stacks, and statistical modes.
- [x] Sandboxed JavaScript/WASM plugin manifest, permissions, panels, filters, exporters, and versioning.
- [x] Trusted local script console, headless worker, deterministic replay, and optional local CLI.
...
- [x] Stable fixture pack and reproducible visual-regression suite.
