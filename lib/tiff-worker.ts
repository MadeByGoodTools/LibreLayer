...
self.onmessage=(event:MessageEvent<{buffer:ArrayBuffer;page?:number}>)=>{try{
 const {buffer,page}=event.data;if(buffer.byteLength>64*1024*1024)throw Error('TIFF import is limited to 64 MB.');
 ...
 if(!Number.isInteger(width)||!Number.isInteger(height)||width<1||height<1||width>8192||height>8192||width*height>24_000_000)throw Error('TIFF page exceeds 8,192 pixels per side or 24 megapixels.');
