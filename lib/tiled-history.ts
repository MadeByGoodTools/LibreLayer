...
export function historyBytes(frames:{surfaces:HistorySurface[]}[]){const seen=new Set<ImageTile>();let bytes=0;for(const frame of frames)for(const surface of frame.surfaces)for(const image of [surface.pixels,surface.mask])if(image)for(const tile of image.tiles)if(!seen.has(tile)){seen.add(tile);bytes+=(tile.bytes?.byteLength??4)+32}return bytes;}
