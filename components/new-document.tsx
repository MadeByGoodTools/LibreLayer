...
export function NewDocumentDialog({open,onOpenChange,onCreate}:{open:boolean;onOpenChange:(open:boolean)=>void;onCreate:(options:NewDocumentOptions)=>void}) {
  ...
  const valid=Number.isInteger(+w)&&Number.isInteger(+h)&&+w>0&&+h>0&&+w<=16384&&+h<=16384&&+w*+h<=64000000;
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto" onKeyDown={e=>e.stopPropagation()}><DialogTitle>New document</DialogTitle><DialogDescription>Choose a preset or enter custom pixel dimensions. RGB, 8-bit.</DialogDescription>
    <form className="grid gap-4" onSubmit={e=>{e.preventDefault();if(!valid)return;try{onCreate({name:name.trim()||'Untitled artwork',w:+w,h:+h,background});setError('');onOpenChange(false)}catch(e){setError(e instanceof Error?e.message:'Could not create this document. Try smaller dimensions.')}}}>
      ...
      <div className="grid grid-cols-2 gap-3"><label className="grid gap-1">Width (px)<input required className="rounded border p-2 min-w-0" type="number" min={1} max={16384} step={1} value={w} onChange={e=>setW(e.target.value)}/></label><label className="grid gap-1">Height (px)<input required className="rounded border p-2 min-w-0" type="number" min={1} max={16384} step={1} value={h} onChange={e=>setH(e.target.value)}/></label></div>
      ...
      <p className="text-muted-foreground">Up to 16,384 pixels per side and 64 megapixels total. The A4 preset sets pixel dimensions, not print metadata.</p>
